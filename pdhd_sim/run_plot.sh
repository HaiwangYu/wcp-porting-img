...
# Produces, for each work/anode<N>-<P>/protodunehd-sp-frames-sim-anode<N>.tar.bz2:
...
Options:
  -a <anode>       Anode index 0..3 (default: all matching dirs)
...
shopt -s nullglob
dirs=( "$PDHD_SIM_DIR"/work/anode${GLOB_ANODE}-${GLOB_PLANE} )
shopt -u nullglob
if [ ${#dirs[@]} -eq 0 ]; then
    echo "no work dirs match anode${GLOB_ANODE}-${GLOB_PLANE} under $PDHD_SIM_DIR/work/" >&2
    exit 1
fi

for d in "${dirs[@]}"; do
    [ -d "$d" ] || continue
    files=( "$d"/protodunehd-sp-frames-sim-anode*.tar.bz2 )
    [ -f "${files[0]}" ] || { echo "[skip] no frame archive in $d" >&2; continue; }
    f="${files[0]}"

    echo "=== $(basename "$d") :: $(basename "$f") ==="
    if [ "$DO_FRAMES" = "1" ]; then
        woodpecker plot-frames "$f" --detector hd --out "${f}.png"
    fi
    if [ "$DO_WAVE" = "1" ]; then
        "$PDHD_SIM_DIR/extract_track_waveform.py" "$f"
    fi
done
