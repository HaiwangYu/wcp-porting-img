#!/usr/bin/env python3
"""Generate per-(anode, plane) track JSON files for ProtoDUNE-HD.
...
Output:
  tracks/tracks-hd-anode<N>-<P>.json   (4x3 = 12 files)

Adapted from /nfs/data/1/xning/wirecell-working/generate_tracks.py — specialised
for HD and parameterised so the geometry path and output dir are configurable.
"""
...
# HD: from anode-x inspection — for anodes 0,2 take faces[0]; for anodes 1,3 take faces[1].
HD_NEAR_FACE_IDX_IN_ANODE = {0: 0, 1: 1, 2: 0, 3: 1}

# Drift offset (mm): how far inside the active drift volume to place the track,
# measured from the wire plane toward the cathode. ~500 mm keeps the track
# clear of the anode plane and inside the field-response region.
# See params.jsonnet (pgrapher/experiment/pdhd) for the geometry these refer to.
...
def main():
    ...
    if len(anodes) != 4:
        raise SystemExit(
            f'ERROR: {args.geom} has {len(anodes)} anodes; ProtoDUNE-HD '
            f'expects 4. Wrong wires file? Default is '
            f'protodunehd-wires-larsoft-v1.json.bz2.'
        )

    print(f'=== ProtoDUNE-HD: {args.geom} -> {args.outdir} ===')
    for ai in range(4):
        face_pos = HD_NEAR_FACE_IDX_IN_ANODE[ai]
        face_idx = anodes[ai]['faces'][face_pos]
        for plane_idx_in_face, plane_label in enumerate(PLANE_LABELS):
            ...
            rec = build_record(ti, ai, plane_label, 'hd')
            out = args.outdir / f'tracks-hd-anode{ai}-{plane_label}.json'
