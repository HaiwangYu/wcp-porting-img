...
locally, with detector and plane-split hard-coded for HD.

Plane split (HD): fixed channel-count offsets 800/1600 within the local
0-based numbering (800 U + 800 V + 960 W per APA).
...
Usage:
    ./extract_track_waveform.py work/anode0-W/protodunehd-sp-frames-sim-anode0.tar.bz2
...
PLANE_LABELS = ["U", "V", "W"]
HD_PLANE_BOUNDARIES = [800, 1600]


def _split_planes_hd(frame, channels):
...
def main():
    ...
    print(f"  detector=hd  target plane={plane}")
    ...
    plane_data = _split_planes_hd(frame, channels)
    if len(plane_data) != 3:
