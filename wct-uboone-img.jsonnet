...
local save_icluster(fname) = g.pnode({
    type: "ClusterFileSink",
    name: fname,
    data: {
        outname: fname,
...
then g.pipeline([
    ...
    local active_cluster_output = save_icluster("active-clusters-anode%d.npz" % anode.data.ident),
    local masked_cluster_output = save_icluster("masked-clusters-anode%d.npz" % anode.data.ident),
    local multipipe = g.intern(
        innodes=[dead_live_fanout],
        outnodes=[active_cluster_output, masked_cluster_output],
        centernodes=[active_fork, masked_fork, active_cluster_output, masked_cluster_output],
        edges=[
            ...
            g.edge(active_fork, active_cluster_output),
            g.edge(masked_fork, masked_cluster_output)
        ],
