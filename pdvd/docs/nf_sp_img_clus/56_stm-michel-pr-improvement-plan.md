...
**Status (2026-09-09, updated same day after T1a). Analysis and plan; §8's
T1a row is now executed — see doc pdvd/57 for its code, gates and results
(both byte-identical gates PASS; the knob stays default OFF, out of both
`wct-pr-perevt.jsonnet`). Everything else below is still analysis: no other
C++ changed, no other knob moved.** It answers the owner's request after doc
pdvd/55 closed the hand scan: read the scan against the reconstruction code,
find the mechanism behind each failure class, pick the candidates to dive into
first, and write down the tasks — one per session — with the intermediate
metric each one is graded on, because the final `is_stm` / `michel_found` flag
is the wrong yardstick for most of them.
...
retreat, its gates and its results).
...
| **T1a** | ✅ **DONE, doc pdvd/57.** stop retreat, graph level: in `CheckSTM_Michel`, after the chain is built, walk *back* from `stop_v` while the trailing chain segment(s) are collapsed (median < 0.5 × plateau) and the profile before them still peaks ≥ 1.4×; mirror of `stop_extend_max` (`:1298`); graph-only in `StmMichelFunctions`, 6 doctests. Result on the real chain graph (not the offline proxy that sized it): **2 of 51** missed collapse-shaped stoppers recovered (`039252_2/39`, `039349_18/36`), **0** regressions, **0** new `is_stm` false positives on the 80-item negative control (both byte-identical gates PASS, PDVD 579/579 + PDHD 325/325). Michel attachment improves as a side effect (role 3 180→185, swallowed 19→13); pin residual median 4.60→3.53 cm. Two THRU items where the retreat fired stayed correctly non-`is_stm` but picked up a spurious `michel_found=1` — feeds T2. Knob `stop_retreat_max` stays OFF in both `wct-pr-perevt.jsonnet` (owner's call to flip) | 2 | (scored, see doc 57 §5) | stop residual 36 pins 4.60→**3.53 cm**, within 2 cm 3→**6**; collapse 51→**48** missed; `is_stm` census (549,144,9,125,271,.941,.535)→(549,146,9,123,271,.942,.543) | knob `stop_retreat_max`; off = byte-identical (both detectors, verified); scorer on/off; zero unconditional SBND exposure (component shared, not yet gated there since T1b is what touches `TaggerCheckSTM`) |
| **T1b** | **the asymmetric kink in `find_first_kink`.** A third clause accepting a kink where the entry-side 10-point density is ≥ 1.2 MIP and the far side ≤ 0.5 MIP (Bragg into Michel), so the tagger's own `kink_num` stops at the muon; and `vertex_kink_reject` (`TaggerCheckSTM.cxx:2474`) evaluating at `n−1` under the sentinel. The gate is ported verbatim, so this is a documented divergence (M15). Doc 57 §1/§7 measured that this clause **still cannot reach 23 of the 51** missed-collapse items — `anchor_vertex` snaps to a vertex within 2 cm, and those 23 have none nearby — so T1b alone will not close the class T1a left; T1c is the piece that can | 2 | same six + the 22 collapse items whose kink sits 1–7 rows from the end | sentinel rate on the 51 (29 today); `T_stm_pass.kink_num` vs scan pin | knob in `TaggerCheckSTM`; SBND `ab_check.sh` PASS with it off; PDHD arm |
| **T1c** | **NEW (found by doc 57). Fit-row split at the retreat point.** 23 of the 51 missed-collapse items have no chain vertex within 25 cm of the stop at all — the collapse sits *inside* the last fitted segment, so no graph-only walk (T1a or T1b) can reach it. Needs `stm_michel_profile`'s own retreat criterion (§2, doc 57) to select a fit ROW rather than an existing vertex, and a real graph split there (new vertex + two segments) before the chain can be truncated — a materially larger change than T1a, touching the Steiner/fit boundary machinery, not just a walk. Dive first on the 23 named in doc 57 §1's probe output | 2 | the 23 no-vertex missed-collapse items (`d57_retreat_probe.py` names them) | same stop-residual / collapse-count metrics as T1a, restricted to the 23 | new knob; graph-split code is a bigger review surface than T1a — plan it as its own session before starting |
| **T2** | **Michel admission at the stop.** Trace the 7 "passes everything" arms and the 8 `michel_mip_lo` failures item by item; decide whether `mip_lo` should read total charge (KE) rather than median dQ/dx, and whether the arm may hang off any vertex within `stop_snap_tol`. Doc 57 §5 adds a concrete new input: a retreated stop is not automatically `is_stm`-confirmed (2 THRU items picked up `michel_found=1` at `conn_type 1, dis 0` when the retreat fired but the shape tests still correctly rejected the track) — T2 should decide whether a Michel arm at a *retreated* stop needs a stronger gate than one at the tagger's original stop | 3 | `039253_17/77` (77007, 0.10 MIP), `039349_22/63`, `039349_5/64`, `039252_0/75`, `039349_32/63`, `039349_7/4`, `039349_82/54`; plus the 2 retreat-caused false Michels `039349_20/41`, `039349_48/21` | Michel attachment (180 role 3 → ?); D 41 → ? | knob; §14.2 purity must not drop |
...
**Order.** T1a (done) → T2 → T3 share the stop and each unlocks the next; T1a
alone touched the class the scan sized largest, and recovered 2 of it outright
with 6 more Michel segments freed as a side effect (doc 57). T1b and T4 are
independent of the graph stage and can run in parallel; **T1c is new** (found
while executing T1a) and is priced above as its own, larger session — it does
not block T2/T3, which can proceed on the stop as T1a leaves it. Then T5, T6,
T7, T8.
