#!/usr/bin/env bash
# doc pdvd/51 -- stage and run the d51g PR arms (the muon-capture gamma at the
# STM stop, and the Michel's role/membership fix) on both ProtoDUNEs.
#
...
# wct-pr-perevt.jsonnet) merges on top of the driver's 15-key default bag, so
# `-S stm_michel_extra={stop_gamma_enable:false}` is provably a one-key change --
...
#
# Usage:
#   ARM=d51gv    DET=pdvd SRC=d16vnu JOBS=8 ./d51g_run_arms.sh
#   ARM=d51gvleg DET=pdvd SRC=d16vnu JOBS=8 \
#       PR_TLA='-S stm_michel_extra={stop_gamma_enable:false}' ./d51g_run_arms.sh
#   ARM=d51gh    DET=pdhd SRC=d16hnu JOBS=8 ./d51g_run_arms.sh
#   ARM=d51ghleg DET=pdhd SRC=d16hnu JOBS=8 \
#       PR_TLA='-S stm_michel_extra={stop_gamma_enable:false}' ./d51g_run_arms.sh
set -uo pipefail
...
JOBS=${JOBS:-8}
PR_TLA=${PR_TLA:-}
LOGD=${LOGD:-/home/xqian/tmp/d51/arm_$ARM}; mkdir -p "$LOGD"
D=$IMG/$DET
...
STAMP=$(mktemp /home/xqian/tmp/d51g_stamp_XXXXXX); trap 'rm -f "$STAMP"' EXIT
