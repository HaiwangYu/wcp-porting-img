#!/bin/bash
# Serve the PDHD PDVD doc-08 STM hand-scan display (doc pdhd/08 sec 9.3).
#
# Forked BY DUPLICATION from pdhd/d08_scan/serve_d08_scan.sh; that script is untouched.
#
# Usage: ./serve_d08pv_scan.sh [port] [--tag NAME]
#   port        (optional, default 5017)
#   --tag NAME  (optional, default 'd08pvflip0') namespaces saved labels into
#               work/d08pv_scan_labels/NAME/labels.json, so a second pass or a
#               second scanner keeps its labels apart.  NEVER reuse a tag whose
#               labels you want to keep -- a scan record is a record (M13).
#
# PORT 5017 IS SHARED with pdhd/stm_scan, pdhd/d05_scan and pdhd/d08_scan.  A second server on a
...
# then open http://localhost:5017/d08pv_scan_viewer
#
# Labels are written on EVERY click to work/d08pv_scan_labels/<tag>/labels.json,
...
exec "$BOKEH" serve --port "$PORT" \
    ...
    "$HERE/d08pv_scan_viewer.py" --args "${TAG_ARGS[@]}"
