#!/usr/bin/env python3
"""PDHD doc-08 STM-flip hand scan -- the 45 objects whose STM verdict moves when
the retiler's path bridge is capped (retile_hack_max_bridge = 10 cm).

Forked BY DUPLICATION from pdhd/stm_scan/stm_scan_viewer.py, which is untouched.

WHAT THIS IS FOR
  doc pdhd/08 sec 8: capping the bridge removes every Steiner ghost beyond 30 cm
  (worst 39.2 -> 11.3 cm) and the TGM tagged set does not move at all -- but 45 of
  174 STM tags change, 21 gained and 24 lost.  Nothing in the numbers says whether
  the cap removed support that was REAL or support that was FABRICATED.  That is a
  physics judgement on the charge, and this app is where it gets made.

TWO MODES, AND THE DIFFERENCE MATTERS
  1. JUDGE (default).  Charge only.  You answer "does this object stop inside?"
     from the charge alone, exactly as in the retile0 scan.  The sheet carries no
     verdict and NOT the direction of the flip -- knowing an object LOST its tag
     is the single most biasing fact available, so it is not on the sheet, not in
     the UI, and not in the item label.
...
WHY THE CHARGE PANELS ARE ARM-INDEPENDENT
  ...
  between the two arms (asserted by selftest_d08_scan.py), so the coloured/grey
  pixels cannot encode which arm produced them.  In 9 of 30 events a handful of
  points (0.005-2.3 %) change which cluster they belong to; for the 2 scan items
  where that touches the item's own cluster the sheet sets partition_moved=1 and
  the header says so.  The panels always draw the BASE arm's partition.
...
USAGE
  ./serve_d08_scan.sh 5017                  # then http://localhost:5017/d08_scan_viewer
  ./serve_d08_scan.sh 5017 --tag pass2      # namespace the saved labels
"""
import sys
import os
import json
...
DENSE_R = 40.0
DENSE_MAX = 25000
BASE_ARM = "d08goff"        # knobs off  -- "before"
KNOB_ARM = "d08cap10"       # retile_hack_max_bridge = 10 cm -- "after"
RUN6 = "029107"
...
LABEL_DIR = os.path.join(WORK, "d08_scan_labels", SCAN_TAG)
...
def reveal_layers(event, arm):
    """(steiner xyz+cid, stm_fit xyz+cid, set of stm_tagged cluster ids)."""
    k = (event, arm)
    if k in _reveal_cache:
        return _reveal_cache[k]
    zp = os.path.join(WORK, "%s_%s_%s" % (RUN6, event, arm), "mabc-pr.zip")
    out = {}
    for name in ("steiner_graph", "stm_fit", "stm_tagged"):
        d = _layer(zp, name)
...
def save_labels():
    ...
    with open(tmp, "w") as fh:
        json.dump({"scan": "doc pdhd/08 retile_hack_max_bridge STM flips",
                   "doc": "pdhd/docs/08_steiner-ghost-fix.md sec 8",
...
<b>PDHD doc-08 STM-flip hand scan</b> &mdash; the 45 objects whose STM verdict moves under
<code>retile_hack_max_bridge = 10&nbsp;cm</code> (doc pdhd/08 &sect;8).
...
of the cluster is drawn. Red dashed = the active boundary. Full detector by default on
purpose &mdash; a cluster zoomed to its own extent looks contained in every view.</span>
...
def draw_reveal(it, axes):
    ...
    def verdict(arm):
        d = reveal_layers(e, arm).get("stm_tagged")
        if d is None:
            return "no stm_tagged layer"
    ...
    reveal_div.text = (
        ...
        % (BASE_ARM, verdict(BASE_ARM), sb, fb, fitfmt(BASE_ARM),
           KNOB_ARM, verdict(KNOB_ARM), sa, fa, fitfmt(KNOB_ARM)))
...
# headless end-to-end test in selftest_d08_scan.py drives the reveal -- silently
...
curdoc().title = "PDHD doc-08 STM-flip scan (%s)" % SCAN_TAG
go(0)
