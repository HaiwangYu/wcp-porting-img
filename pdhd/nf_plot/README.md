...
## L1SP response-kernel validation

| File | Purpose |
|------|---------|
| `track_response_l1sp_kernels.py` | Build and validate the `L1SPFilterPD` response-kernel file. Default (no args): builds kernels in-memory from FR + electronics via `wirecell.sigproc.l1sp.build_l1sp_kernels`, round-trips through a temp JSON+bz2, and asserts bit-identical. `--from-file <path>`: loads a kernel file produced by `wirecell-sigproc gen-l1sp-kernels` and cross-checks against an in-memory FR rebuild (use `--no-rebuild` to skip). Produces three PNGs: uBooNE U/V reference, PDHD U positive/negative, PDHD V positive/negative. |

Typical usage after generating or updating the kernel file:

```bash
cd nf_plot
# validate the installed file (bit-identical cross-check + plots):
direnv exec /nfs/data/1/xqian/toolkit-dev python3 \
    track_response_l1sp_kernels.py \
    --from-file pdhd_l1sp_kernels.json.bz2
```

The plots show:
- **positive-ROI panel**: bipolar (blue) and W-unipolar (orange) — W peak
  should land at the bipolar zero crossing (per-plane W shift is auto-derived).
- **negative-ROI panel**: bipolar (blue) and neg-half (dashed green) — should
  exactly cover the negative lobe.

The kernel file is generated by:
...
Place the output in `wire-cell-data/` so `WIRECELL_PATH` resolution works.
