...
- **Different sensitivity to wire-region-average response.** PDHD's
  ...
  protection and ROI replacement. PDVD now also wires FR⊗ER kernels for
  U/V, but coherent-NF decon threshold tuning (`roi_min_max_ratio`,
  `decon_limit`) for PDVD is deferred and currently uses global defaults.
...
- **Tuning surfaces are not fully portable.** PDHD's per-plane threshold
  overrides (`roi_min_max_ratio 3.0/1.5`, `decon_limit` U/V/W splits)
  have no equivalents in PDVD's current configuration; PDVD's per-plane
  U/V `response` waveforms and `response_offset` are now populated, but
  coherent-NF threshold tuning for PDVD is deferred.
